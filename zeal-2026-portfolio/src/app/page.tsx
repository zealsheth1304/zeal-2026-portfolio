    ...
    <div className="relative">
      ...
      <section className="relative min-h-screen flex flex-col justify-center overflow-hidden px-ds-6 pt-ds-20">
        ...
        <div className="max-w-7xl mx-auto w-full z-10">
          <div className="max-w-4xl">
            <header className="mb-ds-12 overflow-hidden">
              ...
              <h1 className="text-ds-h2 md:text-ds-h1 leading-ds-flat font-bold tracking-ds-tight text-text-main mb-ds-12">
            ...
            <div className="flex flex-col md:flex-row gap-12 items-start md:items-center">
              <p className="text-ds-b2 md:text-ds-b1 text-text-muted max-w-xl leading-relaxed font-medium">
        ...
        <div className="absolute bottom-16 right-10 md:right-20 hidden md:block rotate-90 origin-right">
          <span className="text-ds-c2 uppercase tracking-ds-widest font-bold text-text-muted opacity-30">
            Scroll to discover →
          </span>
        </div>
      </section>
      ...
      <section id="work" className="py-ds-32 px-ds-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-ds-24 gap-ds-8">
          <div>
            <span className="text-ds-c2 uppercase tracking-[0.3em] font-bold text-text-muted mb-ds-4 block">Selected Archive</span>
            <h2 className="text-5xl md:text-7xl font-bold tracking-tight">Curation.</h2>
          </div>
          <p className="max-w-xs text-text-muted text-sm leading-relaxed font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-ds-12 gap-y-ds-32">
          {projects.map((project, idx) => (
            ...
            >
              <div className="relative aspect-[4/5] overflow-hidden rounded-ds-2xl bg-slate-100 dark:bg-slate-900 mb-ds-8">
                <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/10 transition-colors duration-700" />
                <div className="w-full h-full bg-gradient-to-br from-primary/5 to-secondary/5 flex items-center justify-center group-hover:scale-105 transition-transform duration-1000 ease-out">
                  <span className="text-[10px] uppercase tracking-widest font-bold opacity-20">{project.title}</span>
                </div>

                <div className="absolute bottom-ds-8 left-ds-8 right-ds-8 flex justify-between items-end opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-ds-4 group-hover:translate-y-0">
              ...
              <div className="flex flex-col gap-ds-3 w-full">
                <span className="text-[10px] font-mono text-text-muted opacity-40">0{idx + 1}</span>
                ...
                <p className="text-ds-c2 text-text-muted font-medium w-full break-words">{project.description}</p>
                <div className="flex flex-wrap gap-x-ds-4 gap-y-ds-2 mt-ds-1">
                  {project.tags.map((tag) => (
                    <span key={tag} className="text-[9px] uppercase tracking-widest font-black text-text-muted border border-border-strong rounded-ds-full px-ds-2 py-ds-1">
                      {tag}
                    </span>
                  ))}
                </div>
      ...
      <section className="py-ds-32 px-ds-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-ds-20 gap-ds-8">
          <div>
            <span className="text-ds-c2 uppercase tracking-[0.3em] font-bold text-text-muted mb-ds-4 block">Kind Words</span>
