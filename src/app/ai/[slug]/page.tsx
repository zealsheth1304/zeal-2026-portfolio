...
import { getAIBySlug, getAISlugs, getAllAI } from "@/lib/mdx";
...
export default async function AIProjectPage({ params }: { params: Promise<{ slug: string }> }) {
    ...
    const { metadata, content } = getAIBySlug(slug);
    ...
    return (
        <div className="relative min-h-screen">
            <article className="pt-40 pb-20 px-12 md:px-16 max-w-5xl mx-auto">
                ...
                >
                    <ArrowLeft size={14} /> Back to AI Projects
                </Link>
                ...
                <header className="mb-20">
                    ...
                    <p className="text-ds-b1 text-muted mb-8 max-w-4xl leading-relaxed font-medium">
                        {metadata.description}
                    </p>

                    {metadata.tags && metadata.tags.length > 0 && (
                        <div className="flex flex-wrap gap-x-3 gap-y-2 mb-12">
                            {metadata.tags.map((tag) => (
                                <span key={tag} className="text-xs uppercase tracking-widest font-bold text-primary bg-primary/10 rounded-full px-4 py-2">
                                    {tag}
                                </span>
                            ))}
                        </div>
                ...
                <footer className="mt-40 pt-16 border-t border-border-strong/50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                    {(() => {
                        ...
                        return (
                            <Link href={`/ai/${nextProject.slug}`} className="group flex flex-col md:items-end md:ml-auto gap-2">
